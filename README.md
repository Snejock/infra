...
Кроме того, в проекте определены две внешние сети, которые должны существовать заранее:
*   `dwh-net`: Сеть для связи сервисов мониторинга между собой.
*   `ch-net`: Сеть, которая позволяет этому стеку мониторинга "видеть" другие Docker-контейнеры и собирать с них метрики.
...
#### 2. Создать внешние Docker-сети

Сервисы подключаются к внешним сетям `ch-net` и `dwh-net`. Их нужно создать вручную **до** первого запуска:
```bash
docker network create ch-net
docker network create dwh-net
```

#### 3. Создать конфигурацию для Prometheus

Стек ожидает, что конфигурационный файл `prometheus.yml` будет находиться по пути `/media/data/projects/infra/compose/prometheus/config/prometheus.yml`.
...
| **cAdvisor** | `http://<IP-адрес-сервера>:8080` |
